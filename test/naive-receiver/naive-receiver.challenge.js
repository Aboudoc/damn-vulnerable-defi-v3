...
describe("[Challenge] Naive receiver", function () {
  ...
  it("Execution", async function () {
    ...
    const HackFactory = await ethers.getContractFactory(
      "contracts/naive-receiver/HackNaiveReceiver.sol:HackNaiveReceiver",
      player
    );

    this.hack = await HackFactory.deploy(receiver.address, pool.address);

    await this.hack.connect(player).attack();
  });
