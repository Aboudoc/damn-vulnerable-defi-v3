...
describe("[Challenge] Free Rider", function () {
  ...
  it("Execution", async function () {
    ...
    //address _nft, address payable _marketplace, address _recovery, address _pair, address payable _weth
    const freeRider = await (
      await ethers.getContractFactory("FreeRider", player)
    ).deploy(
      nft.address,
      marketplace.address,
    ...
    await freeRider.connect(player).flashLoan();

    await freeRider.connect(player).recoverNft();
  });
